...
**Write — variants & component properties:** `combine_as_variants` (merge components named
`Prop=Value` into one set), `set_variant_properties` (set a variant's axis values),
`create_component_property` (add `BOOLEAN` / `TEXT` / `INSTANCE_SWAP` props).

**Write — style authoring (token-driven):** `create_paint_style`, `create_text_style`,
`create_effect_style`, `create_grid_style` — create shared styles instead of hardcoding values.

> For the step-by-step component → variant → token → verify workflow, use the
> **`figma-design-flow`** skill. For semantic token rules, **`shadcn-design-tokens`**.
