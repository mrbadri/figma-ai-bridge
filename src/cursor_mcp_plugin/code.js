...
async function handleCommand(command, params) {
  switch (command) {
    ...
    // --- extended tool surface (proposal §4.2) ---
    case "create_ellipse":
...
// ============================================================================
// Extended tool surface (proposal §4.2): shapes, gradients/images/effects,
// variables/tokens, styles, structural ops, componentization.
// All handlers return plain JSON-serializable objects and reference nodes by id
// (proposal §4.3). Function declarations are hoisted, so the dispatcher cases
// above resolve to these regardless of file position.
// ============================================================================

// --- shared color/paint helpers ---
function _rgb(c) {
  c = c || {};
  return {
    r: parseFloat(c.r) || 0,
...
function _solidPaint(c) {
  return {
    type: "SOLID",
    color: _rgb(c),
    opacity: c && c.a !== undefined ? parseFloat(c.a) : 1,
  };
}

async function _resolveParent(parentId) {
...
// Decode a base64 string to bytes for figma.createImage. Prefers the native
// figma.base64Decode when available, otherwise decodes manually (the plugin
// main thread has no atob).
...
async function createEllipse(params) {
  const { x = 0, y = 0, width = 100, height = 100, name = "Ellipse", parentId, fillColor } = params || {};
  const node = figma.createEllipse();
  node.x = x; node.y = y; node.resize(width, height); node.name = name;
  if (fillColor) node.fills = [_solidPaint(fillColor)];
  (await _resolveParent(parentId)).appendChild(node);
  return _baseGeometry(node);
}

async function createLine(params) {
  const { x = 0, y = 0, length = 100, rotation = 0, strokeColor, strokeWeight = 1, name = "Line", parentId } = params || {};
  const node = figma.createLine();
  node.x = x; node.y = y; node.resize(length, 0); node.rotation = rotation; node.name = name;
  node.strokes = [_solidPaint(strokeColor || { r: 0, g: 0, b: 0, a: 1 })];
...
async function createPolygon(params) {
  const { x = 0, y = 0, width = 100, height = 100, pointCount = 3, name = "Polygon", parentId, fillColor } = params || {};
  const node = figma.createPolygon();
  node.pointCount = Math.max(3, pointCount);
  node.x = x; node.y = y; node.resize(width, height); node.name = name;
  if (fillColor) node.fills = [_solidPaint(fillColor)];
  (await _resolveParent(parentId)).appendChild(node);
  return _baseGeometry(node);
...
async function createStar(params) {
  const { x = 0, y = 0, width = 100, height = 100, pointCount = 5, innerRadius = 0.5, name = "Star", parentId, fillColor } = params || {};
  const node = figma.createStar();
  node.pointCount = Math.max(3, pointCount);
  node.innerRadius = Math.min(1, Math.max(0, innerRadius));
  node.x = x; node.y = y; node.resize(width, height); node.name = name;
  if (fillColor) node.fills = [_solidPaint(fillColor)];
  (await _resolveParent(parentId)).appendChild(node);
  return _baseGeometry(node);
...
// Import an SVG string as editable vectors (proposal §4.2 vector; the
// figma-generate-design skill mandates this path for icons over redrawing).
async function createSvg(params) {
  const { svg, x = 0, y = 0, width, height, name = "SVG", parentId } = params || {};
  if (!svg) throw new Error("Missing svg parameter");
  const node = figma.createNodeFromSvg(svg);
  node.x = x; node.y = y; node.name = name;
  if (width && height) node.resize(width, height);
  (await _resolveParent(parentId)).appendChild(node);
  return _baseGeometry(node);
}

// --- appearance: gradient / image / effects ---
async function setGradientFill(params) {
  const { nodeId, gradientType = "GRADIENT_LINEAR", stops, gradientTransform } = params || {};
  if (!nodeId) throw new Error("Missing nodeId parameter");
  if (!Array.isArray(stops) || stops.length < 2)
    throw new Error("Provide at least two gradient stops");
  const node = await figma.getNodeByIdAsync(nodeId);
  if (!node) throw new Error(`Node not found with ID: ${nodeId}`);
...
async function setImageFill(params) {
  ...
  if (!nodeId) throw new Error("Missing nodeId parameter");
  if (!imageData) throw new Error("Missing imageData (base64) parameter");
  const node = await figma.getNodeByIdAsync(nodeId);
  if (!node) throw new Error(`Node not found with ID: ${nodeId}`);
  if (!("fills" in node)) throw new Error(`Node does not support fills: ${nodeId}`);
  const image = figma.createImage(_decodeBase64(imageData));
  node.fills = [{ type: "IMAGE", scaleMode, imageHash: image.hash }];
  return { id: node.id, name: node.name, imageHash: image.hash, scaleMode };
}
...
async function getVariables() {
  ...
  for (const collection of collections) {
    ...
    for (const id of collection.variableIds) {
      ...
      if (v) variables.push({ id: v.id, name: v.name, key: v.key, resolvedType: v.resolvedType, valuesByMode: v.valuesByMode });
    }
    result.push({
      id: collection.id,
...
async function createVariable(params) {
  ...
  if (value !== undefined && value !== null) {
    ...
    if (resolvedType === "COLOR") {
      modeValue = {
        r: parseFloat(value.r) || 0,
        g: parseFloat(value.g) || 0,
...
// setBoundVariableForPaint; everything else uses node.setBoundVariable.
async function setBoundVariable(params) {
  const { nodeId, field, variableId } = params || {};
  if (!nodeId || !field || !variableId) throw new Error("Missing nodeId, field, or variableId");
  const node = await figma.getNodeByIdAsync(nodeId);
  if (!node) throw new Error(`Node not found with ID: ${nodeId}`);
...
async function applyStyle(params) {
  const { nodeId, styleType, styleId, styleKey } = params || {};
  if (!nodeId || !styleType) throw new Error("Missing nodeId or styleType");
  const node = await figma.getNodeByIdAsync(nodeId);
  if (!node) throw new Error(`Node not found with ID: ${nodeId}`);
  let id = styleId;
  if (!id && styleKey) {
  ...
  if (!id) throw new Error("Provide styleId or styleKey");
  switch (styleType) {
    case "text":
      if ("setTextStyleIdAsync" in node) await node.setTextStyleIdAsync(id);
      else node.textStyleId = id;
      break;
    case "effect":
      if ("setEffectStyleIdAsync" in node) await node.setEffectStyleIdAsync(id);
      else node.effectStyleId = id;
      break;
    case "fill":
      if ("setFillStyleIdAsync" in node) await node.setFillStyleIdAsync(id);
      else node.fillStyleId = id;
      break;
    case "stroke":
      if ("setStrokeStyleIdAsync" in node) await node.setStrokeStyleIdAsync(id);
      else node.strokeStyleId = id;
      break;
    default:
      throw new Error(`Unknown styleType: ${styleType}`);
  }
  return { id: node.id, name: node.name, styleType, appliedStyleId: id };
}

// --- structural ops ---
async function groupNodes(params) {
  const { nodeIds, name = "Group" } = params || {};
  if (!Array.isArray(nodeIds) || nodeIds.length < 1) throw new Error("nodeIds must be a non-empty array");
  const nodes = [];
...
async function ungroupNode(params) {
  ...
  return { success: true, childIds: children.map((c) => c.id) };
}

async function reorderNode(params) {
  const { nodeId, index, position } = params || {};
  if (!nodeId) throw new Error("Missing nodeId parameter");
  const node = await figma.getNodeByIdAsync(nodeId);
  if (!node) throw new Error(`Node not found with ID: ${nodeId}`);
  const parent = node.parent;
  if (!parent || !("children" in parent)) throw new Error("Node has no reorderable parent");
  const count = parent.children.length;
  let target = index;
  if (position) {
    const cur = parent.children.indexOf(node);
    if (position === "front") target = count - 1;
    else if (position === "back") target = 0;
    else if (position === "forward") target = Math.min(count - 1, cur + 1);
    else if (position === "backward") target = Math.max(0, cur - 1);
  }
  if (target === undefined) throw new Error("Provide index or position");
  parent.insertChild(Math.max(0, Math.min(count - 1, target)), node);
  return { id: node.id, name: node.name, index: parent.children.indexOf(node) };
}

async function booleanOperation(params) {
  const { nodeIds, operation = "UNION", name } = params || {};
  if (!Array.isArray(nodeIds) || nodeIds.length < 2) throw new Error("nodeIds must contain at least two nodes");
  const nodes = [];
  for (const id of nodeIds) {
    const n = await figma.getNodeByIdAsync(id);
    if (n) nodes.push(n);
  }
  if (nodes.length < 2) throw new Error("Need at least two valid nodes");
  const parent = nodes[0].parent || figma.currentPage;
  const ops = { UNION: "union", SUBTRACT: "subtract", INTERSECT: "intersect", EXCLUDE: "exclude" };
  const fn = ops[operation];
  if (!fn) throw new Error(`Unknown boolean operation: ${operation}`);
  const result = figma[fn](nodes, parent);
  if (name) result.name = name;
  return _baseGeometry(result);
}

// Turn an existing node into a main component (componentization, §4.2 / the
// figma-generate-design "build-once-place-instances" pattern).
