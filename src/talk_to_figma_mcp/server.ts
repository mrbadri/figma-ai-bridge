...
// ============================================================================
// Extended tool surface (proposal §4.2). Each forwards to the plugin command of
// the same name and returns the structured result/error verbatim so the model
// can self-correct (proposal §4.3).
// ============================================================================
...
function ok(label: string, result: unknown) {
  return { content: [{ type: "text" as const, text: `${label}: ${JSON.stringify(result)}` }] };
}
...
server.tool(
  ...
  async (p: any) => {
    try { return ok("Created ellipse", await sendCommandToFigma("create_ellipse", p)); }
    catch (e) { return fail("creating ellipse", e); }
  }
);
...
server.tool(
  ...
  async (p: any) => {
    try { return ok("Created line", await sendCommandToFigma("create_line", p)); }
    catch (e) { return fail("creating line", e); }
  }
);
...
server.tool(
  ...
  {
    x: z.number(), y: z.number(), width: z.number(), height: z.number(),
    pointCount: z.number().int().min(3).optional().describe("Number of sides (>=3)"),
    name: z.string().optional(),
    parentId: z.string().optional(),
    fillColor: colorSchema.optional(),
  },
  async (p: any) => {
    try { return ok("Created polygon", await sendCommandToFigma("create_polygon", p)); }
    catch (e) { return fail("creating polygon", e); }
  }
);
...
server.tool(
  ...
  "Create a star in Figma",
  {
    ...
    innerRadius: z.number().min(0).max(1).optional().describe("Inner radius ratio 0..1"),
    name: z.string().optional(),
    parentId: z.string().optional(),
    fillColor: colorSchema.optional(),
  },
  async (p: any) => {
    try { return ok("Created star", await sendCommandToFigma("create_star", p)); }
    catch (e) { return fail("creating star", e); }
  }
);

server.tool(
  "create_svg",
  "Import an SVG string as editable vectors. Preferred path for icons — pass the codebase's own SVG (must include width/height + viewBox).",
  {
    svg: z.string().describe("Full SVG markup, including width/height and viewBox"),
  ...
  async (p: any) => {
    try { return ok("Imported SVG", await sendCommandToFigma("create_svg", p)); }
    catch (e) { return fail("importing SVG", e); }
  }
);

server.tool(
  "set_gradient_fill",
  "Set a gradient fill on a node (linear/radial/angular/diamond)",
  {
    nodeId: z.string(),
    gradientType: z.enum(["GRADIENT_LINEAR", "GRADIENT_RADIAL", "GRADIENT_ANGULAR", "GRADIENT_DIAMOND"]).optional(),
    stops: z.array(z.object({ position: z.number().min(0).max(1), color: colorSchema })).min(2),
    gradientTransform: z.array(z.array(z.number())).optional().describe("2x3 affine transform; defaults to identity"),
  },
  async (p: any) => {
    try { return ok("Set gradient fill", await sendCommandToFigma("set_gradient_fill", p)); }
    catch (e) { return fail("setting gradient fill", e); }
  }
);

server.tool(
  "set_image_fill",
  "Set an image fill on a node from base64-encoded image bytes (uses figma.createImage)",
  {
    nodeId: z.string(),
    imageData: z.string().describe("Base64-encoded image bytes (PNG/JPG)"),
    scaleMode: z.enum(["FILL", "FIT", "CROP", "TILE"]).optional(),
  },
  async (p: any) => {
    try { return ok("Set image fill", await sendCommandToFigma("set_image_fill", p)); }
    catch (e) { return fail("setting image fill", e); }
  }
);

server.tool(
  "set_effects",
  "Set effects (drop/inner shadow, layer/background blur) on a node. Replaces existing effects.",
  {
    nodeId: z.string(),
  ...
  async (p: any) => {
    try { return ok("Set effects", await sendCommandToFigma("set_effects", p)); }
    catch (e) { return fail("setting effects", e); }
  }
);

server.tool(
  "get_variables",
  "Read all local variable collections, modes, and variables (design tokens) in the file",
  {},
  async () => {
    try { return ok("Variables", await sendCommandToFigma("get_variables")); }
    catch (e) { return fail("getting variables", e); }
  }
);

server.tool(
  "create_variable",
  "Create a design-token variable in a collection (creating the collection if needed)",
  {
    collectionName: z.string(),
    variableName: z.string(),
    resolvedType: z.enum(["COLOR", "FLOAT", "STRING", "BOOLEAN"]).optional(),
    value: z.any().optional().describe("Value for the default mode; COLOR expects {r,g,b,a}"),
    modeName: z.string().optional(),
  },
  async (p: any) => {
    try { return ok("Created variable", await sendCommandToFigma("create_variable", p)); }
    catch (e) { return fail("creating variable", e); }
  }
);

server.tool(
  "set_bound_variable",
  "Bind a variable to a node field. Use field 'fills' or 'strokes' for colors; otherwise any bindable field (e.g. 'paddingLeft', 'itemSpacing', 'topLeftRadius').",
  {
    nodeId: z.string(),
    field: z.string(),
    variableId: z.string(),
  ...
  async (p: any) => {
    try { return ok("Bound variable", await sendCommandToFigma("set_bound_variable", p)); }
    catch (e) { return fail("binding variable", e); }
  }
);

server.tool(
  "apply_style",
  "Apply a shared style to a node by id or library key (text/effect/fill/stroke)",
  {
    nodeId: z.string(),
    styleType: z.enum(["text", "effect", "fill", "stroke"]),
    styleId: z.string().optional(),
    styleKey: z.string().optional().describe("Library style key; imported if styleId not given"),
  },
  async (p: any) => {
    try { return ok("Applied style", await sendCommandToFigma("apply_style", p)); }
    catch (e) { return fail("applying style", e); }
  }
);
...
server.tool(
  ...
  { nodeIds: z.array(z.string()).min(1), name: z.string().optional() },
  async (p: any) => {
    try { return ok("Grouped nodes", await sendCommandToFigma("group_nodes", p)); }
    catch (e) { return fail("grouping nodes", e); }
  }
);

server.tool(
  "ungroup_node",
  "Ungroup a group or frame, returning the freed child node ids",
  { nodeId: z.string() },
  async (p: any) => {
    try { return ok("Ungrouped node", await sendCommandToFigma("ungroup_node", p)); }
    catch (e) { return fail("ungrouping node", e); }
  }
);

server.tool(
  "reorder_node",
  "Change a node's z-order within its parent, by explicit index or position keyword",
  {
    nodeId: z.string(),
  ...
  async (p: any) => {
    try { return ok("Reordered node", await sendCommandToFigma("reorder_node", p)); }
    catch (e) { return fail("reordering node", e); }
  }
);

server.tool(
  "boolean_operation",
  "Combine two or more vector/shape nodes with a boolean operation",
  {
    nodeIds: z.array(z.string()).min(2),
    operation: z.enum(["UNION", "SUBTRACT", "INTERSECT", "EXCLUDE"]).optional(),
    name: z.string().optional(),
  },
  async (p: any) => {
    try { return ok("Boolean operation", await sendCommandToFigma("boolean_operation", p)); }
    catch (e) { return fail("running boolean operation", e); }
  }
);

server.tool(
  "create_component_from_node",
  "Turn an existing node into a reusable main component (componentization)",
  { nodeId: z.string(), name: z.string().optional() },
  async (p: any) => {
    try { return ok("Created component", await sendCommandToFigma("create_component_from_node", p)); }
    catch (e) { return fail("creating component", e); }
  }
);
